...
#preparing the data 
dds <- transcript_data 
dds <- select(dds, c(1,6:length(dds))) #selects the sample_ID and gene data, add '3' for group column data 

#dds <- dds[order(dds$sample_ID), ]
#dds <- select(dds, contains('_dose1')) #selecting FC data for first dose genes

ads <- dds # creating a second object for transcript data analysis
ads <- column_to_rownames(ads, var = "sample_ID")
ads <- select(dds, contains('_dose1'))
...
res.ind <- get_pca_ind(pcaRes)
res.ind$coord          # Coordinates
res.ind$contrib        # Contributions to the PCs
res.ind$cos2           # Quality of representation

# Extract loadings and eigenvalues
loadings <- pcaRes$rotation  # Loadings (variable loadings on PCs)
eigenvalues <- pcaRes$sdev^2  # Eigenvalues (variance explained by each PC)
...
ads <- as.data.frame.data.frame(dds)
names <- ads$sample_ID
ads <- column_to_rownames(ads, var = "sample_ID")
ads <- dplyr::select(ads, contains('_dose1'))

# Calculate the variance for each column
variance_values <- apply(ads, 2, var, na.rm = TRUE)

# Determine the number of columns to select (top 10%)
percentage_to_select <- 0.1
num_columns_to_select <- round(percentage_to_select * ncol(ads))
...
ads_filtered <- ads[, selected_columns, drop = FALSE]

# Print the filtered data frame
print(ads_filtered)

ads_filtered$sample_ID <- namesx #this might need to be changed
```
...
trans_dtz <- pivot_longer(ads_filtered,
    cols = !sample_ID,                      
    names_to = "feature",
    values_to = "value",
  )

trans_dtz <- plyr::rename(trans_dtz, c("sample_ID" = "sample")) #this one worked to rename them
trans_dtz <- cbind(trans_dtz, view = "mRNA" )

#cytokine data processing, choosing sample with timepoint
time <- "V2"
cyto_x <- cytokine_data %>% dplyr::filter(Timepoint %in% time)
cyto_v <- select(cyto_x, c(1, 5:35)) #takes the subject ID and the cytokines

cyto_hist <- select(cyto_v, c(2:32))
hist(as.matrix(cyto_hist))

#Resetting group values & names
cyto_v[cyto_v == "Experienced"] <- "pos"
cyto_v[cyto_v == "Naive"] <- "neg"
cyto_v <- plyr::rename(cyto_v, c("subject_ID" = "sample"))
cyto_v <- plyr::rename(cyto_v, c("Study group" = "group"))


#Matches samples against against the transcript data
names_match <- dds$sample_ID
cyto_v <- cyto_v %>%
...
dt <- bind_rows(trans_dtz, cyto_dtv)
...
MOFAobject <- create_mofa(dt_filter)
...
ggplot()
#trans_dtz: pull top 10 genes & make scatter plot, don't forget to scale...
