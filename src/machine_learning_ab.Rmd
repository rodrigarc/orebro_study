...
#Filter low counts
keep_genes <- rowSums( counts_raw > .5 ) >= 10
...
rpkm_plot <- counts_filtered_vst %>% 
  as.data.frame() %>%
  tibble::rownames_to_column("gene_symbol") %>%
  tidyr::pivot_longer(!gene_symbol, values_to = "counts") %>%
...
## Network analysis of genes highly correlated to Spike fold change

```{r}

corr_fold_changes <- fold_changes %>% 
  t()

## Compare only specific_dose
#corr_fold_changes <- corr_fold_changes[grep("dose3",rownames(corr_fold_changes)),]

corr_fold_changes <- corr_fold_changes[,-grep("ENSG", colnames(corr_fold_changes))]

corr_val_pfizer <- cor(corr_fold_changes[,-grep("vaccine", colnames(corr_fold_changes))],
    corr_fold_changes[,grep("vaccine", colnames(corr_fold_changes), value = T)], method = "pearson")

#corr_val_pfizer <- psych::corr.test(corr_fold_changes[,-grep("vaccine", colnames(corr_fold_changes))],
#    corr_fold_changes[,grep("vaccine", colnames(corr_fold_changes), value = T)], method = "pearson")

#psych::corPlot(corr_val_pfizer)

corr_val_pfizer_filt <- corr_val_pfizer[abs(corr_val_pfizer) > .4,]
pos_corr <- sort(corr_val_pfizer_filt[corr_val_pfizer_filt > 0], decreasing = TRUE)
neg_corr <- sort(corr_val_pfizer_filt[corr_val_pfizer_filt < 0], decreasing = TRUE)
names(pos_corr)
names(neg_corr)
...
Genes correlated correlated to spike protein levels in blood (>.4)

```{r}
process_to_gsea <- function(x, decreasing = TRUE) {
  if(is.matrix(x)){
    original_gene_list <- as.numeric(x)
    names(original_gene_list) <- rownames(x)
    gene_list <- na.omit(original_gene_list)
    gene_list <- sort(gene_list, decreasing = decreasing)
  } else {
    original_gene_list <- as.numeric(x)
    names(original_gene_list) <- names(x)
    gene_list <- na.omit(original_gene_list)
    gene_list <- sort(gene_list, decreasing = decreasing)
}}

pos_list <- process_to_gsea(pos_corr)
neg_list <- process_to_gsea(neg_corr, decreasing = FALSE)
all_list <- process_to_gsea(corr_val_pfizer_filt)
gene_list <- process_to_gsea(corr_val_pfizer)


```

### Overrepresentation analysis of genes correlated to spike protein in serum

```{r plot_enrichemnt}
# Set desired organism to homo sapiens
organism <- "org.Hs.eg.db"
# BiocManager::install(organism, character.only = TRUE)
library(organism, character.only = TRUE)


go_enrich <- clusterProfiler::enrichGO(gene = names(all_list),
...
clusterProfiler::dotplot(go_enrich)
cnetplot(go_enrich, categorySize="pvalue", foldChange=gene_list)

ggsave(paste0("../",result.dir,"network_analysis_correlated_genes_spike.pdf"),
...
for(doses in c("_dose1", "_dose2")){
  ...
  trainIndex <- createDataPartition(seq_len(nrow(fold_changes_ab_data_dt)), p = .8,
                                    list = FALSE,
                                    times = 1,
                                    group = as.integer(fold_changes_ab_data$group))
 #   if(doses == "_dose1"){
