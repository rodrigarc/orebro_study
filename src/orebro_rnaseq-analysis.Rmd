...
dds <- DESeqDataSetFromMatrix(countData = counts_filtered,
                              ...
                              design = ~ group + sex + age + dose)
# Normalize with DESeq
d <- DESeq2::estimateSizeFactors(dds,type="ratio")
counts_DESeq2_normalized <- log2( counts(d,normalized=TRUE) + 1 ) 
saveRDS(counts_DESeq2_normalized,"../data/test_data/gene_counts_normalized_deseq2.Rds")
...
hist(vst_matrix)
hist(counts_DESeq2_normalized)
boxplot(vst_matrix ,ylab=expression('Log'[2]~'Read counts'),las=2,main="VST")
boxplot(counts_DESeq2_normalized ,ylab=expression('Log'[2]~'Read counts'),las=2,main="DESeq2")
```
...
volcanoPlot <- ggplot(res$table, aes(x = logFC, y = -log10(FDR),
                                     color = ifelse(FDR < 0.05 & abs(logFC) > 1, "darkred", "grey"x))) +
  geom_point() +
