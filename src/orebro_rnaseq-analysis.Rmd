...
names(results_list) <- c("neg_dose1", "pos_dose1", "neg_dose2", "pos_dose2","pos","neg", "dose1", "dose2") 
  ...
  x <- x %>% mutate(test = ifelse(padj < 0.05 & abs(log2FoldChange) > 1, "TRUE", "FALSE"))
  })
# color by group dose1 and dose2, or neg and pos. 2 plots colored based on condition
lapply(results_list_plot, function(x) 
  ggplot(x, aes(x=log2FoldChange,y=-log10(padj))) +
...
enrichment_list <- lapply(results_list_plot, function(x) {
  enrichr(
  genes = rownames(x[x$test == TRUE, ]),
