...
The gene sets used were the Blood Transcriptome modules presented in [Li et al. 2014](10.1038/ni.2789).
...
Gene set enrichment analysis based on fold change ranking using the blood transcriptome modules presented in [Li et al. 2014](10.1038/ni.2789). 
...
Pearson’s correlation of fold changes in individual genes identified as differentially regulated in any group. 
N = 15.
Comparison between dose 2 and dose 3.
...
Visualization of specific signficantly increased/decreased cytokines to check their kinetics.
...
The vaccine sequences were added as genes ([uploaded by Jeong et al. on github here on April 15, 2021](https://github.com/NAalytics/Assemblies-of-putative-SARS-CoV2-spike-encoding-mRNA-sequences-for-vaccines-BNT-162b2-and-mRNA-1273)) to the human genome before alignment to identify vaccine-specific mRNA reads. Moderna, Wild spike protein, and wild N-protein were also added as controls and can be visualized with the code below prior filtering pfizer-specific read counts. 

```{r, results='hide'}

# load sample table from salmon output normalized by transcripts per million (tpm)
...
# filter for pfizer specific read row, otherwise keep to visualize moderna, N- and S- proteins
vaccine_plot %>%
