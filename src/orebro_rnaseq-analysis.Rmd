...
# Remove one sample from subject 7 (NA values redundant sample)
...
# all.equal(colnames(counts_raw), rownames(sampleTable)) -> "97 string mismatches", need to convert dataframe to same class?
```
...
```{r}
# Filter low count genes, cutoff: minimum of 5 reads in at least 8 samples
...
```{r}
# prepare for DESeq
# create new variable (conditions) based on previous infection, dose and time point
...
# create function for makings comparisons with dds1 object
compare_DEGs <- function(variable1, variable2) {
  as.data.frame(results(dds1, contrast = c("conditions", variable1, variable2)))
} 
...
# function for dds2 object
compare_DEGs2 <- function(variable1, variable2) {
  as.data.frame(results(dds2, contrast = c("time", variable1, variable2)))
}
# Results dds2
pos <- compare_DEGs2("pos_24", "pos_0")
neg <- compare_DEGs2("neg_24", "neg_0")

# function for dds3
compare_DEGs3 <- function(variable1, variable2) {
  as.data.frame(results(dds3, contrast = c("dose", variable1, variable2)))
}
# Results dds3
d1 <- compare_DEGs3("24_dose1", "0_dose1")
d2 <- compare_DEGs3("24_dose2", "0_dose2")
d3_24 <- compare_DEGs3("24_dose3", "0_dose3")
d3_48 <- compare_DEGs3("48_dose3", "0_dose3")
```
...
volcano_plot <-  function(x){
  ...
  cowplot::theme_cowplot(font_size = 6, 
                         ...
                         font_family = "Arial",
                         rel_large = 4) +
  theme(legend.position = 'none', axis.title = element_text(size = 35), axis.text = element_text(size = 20))
  }
...
degs_names <- unique(data.table::rbindlist(degs_names))[[1]]
degs_names
...
```{r venn_diagram}
to_venn <- list(neg_dose1 = "", 
                pos_dose1 = degs_names[[2]][[1]],
                neg_dose2 = degs_names[[3]][[1]], 
                pos_dose2 = degs_names[[2]][[1]])

ggVennDiagram(to_venn)
```
...
rpkm_plot %>%
  filter(gene_symbol %in% c("IL1RN", "IL1B", "IL1A", "IL1R1", "CXCL10", "IL1A")) %>%
  ggplot(aes(y = RPKM, x = conditions, group = subject_ID, color = subject_ID)) +
  geom_point()+
  geom_line() +
  scale_y_log10() +
  cowplot::theme_cowplot()+
