...
ggsave(g1,filename = paste0("../", result.dir,"linear_relationship_fold_change.pdf"),
      height = 7, width = 7)
```
...
row_col_time <- c("Dose 1 0-24 hours" = "#6A83B5", 
                  "Dose 2 0-24 hours" = "#CC8963",
                  "Dose 3 0-24 hours" = "#7B9E87",
                  "Dose 3 0-48 hours" = "#A695C7")
...
fiss <- plotCounts(dds1,  "ENSG00000289768", 
...
```{r}
degs_entrez_id <- lapply(degs, function(x) plyr::mapvalues(x, from = annot$SYMBOL, to = annot$ENTREZID))

ora_tf_compare <- compareCluster(degs, fun = "enrichPathway")
xx <- compareCluster(degs_entrez_id, fun='enrichKEGG', organism='hsa')
