...
# Results dds1
res_baseline <- as.data.frame(results(dds1, contrast = c("conditions", "Pos_0_dose1", "Neg_0_dose1")))
...
results_list <- list(res_baseline, res_neg_dose1, res_pos_dose1, res_neg_dose2, res_pos_dose2, res_pos, res_neg, res_dose1, res_dose2)

names(results_list) <- c("Baseline", "Naïve Dose 1", "Conv Dose 1", "Naïve Dose 2", "Conv Dose 2", "Conv","Naïve", "Dose 1", "Dose 2") 
    results_list_plot <- lapply(results_list, function(x) {
  x <- x %>% mutate(test = case_when(padj < 0.05 & log2FoldChange >= 1 ~ "Up regulated",
...
.volcano_plot <-  function(x){
  subsetted <- subset(results_list_plot[[x]] %>% tibble::rownames_to_column("gene_symbol"), abs(log2FoldChange) >= 3 & padj < 0.05)
  
  # count up regulated DEGs
  deg_number_up <- subset(results_list_plot[[x]] %>% tibble::rownames_to_column("gene_symbol"), log2FoldChange >= 1 & padj < 0.05) %>%
    nrow()
  # count down regulated DEGs
  deg_number_down <- subset(results_list_plot[[x]] %>% tibble::rownames_to_column("gene_symbol"), log2FoldChange <= -1 & padj < 0.05) %>%
    nrow()
  
  results_list_plot[[x]] %>% 
  ggplot(aes(x=log2FoldChange,y=-log10(padj))) +
  geom_point(aes(colour=test), size=3, alpha=0.3) +
...
#  geom_label_repel(data = subsetted, aes(log2FoldChange,-log10(padj),label= gene_symbol), max.overlaps = 10) +
  xlim(-2,6) +
  ylim(0,10) +
  ggtitle(x) +  
  annotate(
    ...
    annotate(
        ...
        x= -2, y= 10,
        hjust=0,
        label=paste0('n=',deg_number_down)
    ) +
  cowplot::theme_cowplot(font_size = 20, line_size = 1.5, rel_small = 1, rel_tiny = 4,font_family = "Arial") +
...
enrichment_list_up <- lapply(results_list_plot, function(x) {
  enrichr(
  genes = rownames(x[x$test == "Up regulated", ]),
...
enrichment_list_down <- lapply(results_list_plot, function(x) {
  enrichr(
  genes = rownames(x[x$test == "Down regulated", ]),
  databases = c("GO_Biological_Process_2021", "KEGG_2021_Human","GO_Molecular_Function_2021"))})
...
rpkm_filtered <- rpkm(counts_filtered, gene.length = edb_gene_length[edb_gene_length$symbol %in% rownames(counts_filtered),]$gene_length)
...
extract_degs_names <-  function(x){
  subsetted <- results_list_plot[[x]] %>% tibble::rownames_to_column("gene_symbol") %>%
    filter(log2FoldChange > .5 & padj < 0.05) %>%
    select(gene_symbol)
  return(subsetted)
}
