...
heatmap1 <- Heatmap(filtered_degs_matrix_ordered, border = FALSE, 
...
ggsave(g, filename = paste0("../", result.dir, gene_interest,"_cytokine_degs.pdf"),
      height = 5, width = 5)
}
...
p4 <- emapplot(cluster_termsim, cex_category=1.5, layout="nicely", showCategory = 31)
p4 + scale_fill_npg() 

ggsave(filename = paste0("../", result.dir,"ora_kegg_pathway_network.pdf"),
      height = 10, width = 10)

compared_cluster_cnet <- setReadable(compared_cluster, OrgDb=org.Hs.eg.db, "ENTREZID")
cnetplot(compared_cluster_cnet, showCategory = c("NOD-like receptor signaling pathway",
...
ggsave(filename = paste0("../", result.dir,"ora_kegg_pathway_gene_network_selected.pdf"),
      height = 10, width = 10)
...
data_empty <- data.frame( 
     ...
     qvalue = 1,
     geneID = 0,
     Count = 1, GeneRatio = 0, BgRatio = 0) %>% distinct()
...
# Set factor levels
cluster_termsim@compareClusterResult$Cluster <- factor(cluster_termsim@compareClusterResult$Cluster, levels = c("Conv Dose 1 0-24 hours", "Naïve Dose 1 0-24 hours",
                                                       "Conv Dose 2 0-24 hours", "Naïve Dose 2 0-24 hours", 
                                                       "Conv Dose 3 0-24 hours", "Naïve Dose 3 0-24 hours"))


treeplot(cluster_termsim, showCategory = 31,
         label_format = 30, 
         offset.params = list(tiplab = 9,
                              bar_tree = 23), 
...
ggsave(filename = paste0("../", result.dir,"ora_kegg_pathway_tree.pdf"),
      height = 10, width = 30)

dotplot(cluster_termsim, showCategory = 31, color = "log10_padj") +
  theme_prism(base_line_size = .5, base_fontface = "plain", axis_text_angle = 45) +
  labs(x = "", color = "Adjusted p-value\n(-log10)") +
  scale_color_gradient2(low = "#265891", mid = "white", high = "#B80F20") +
  theme(axis.ticks = element_line(size = .5), legend.position = "right",
        aspect.ratio = 1,
        legend.title = element_text())

ggsave(filename = paste0("../", result.dir,"ora_kegg_pathway_dotplot.pdf"),
      height = 15, width = 15)
```
