...
gene_id_metadata <- read.csv("../data/metadata/2023-11-222_mart_export_gene_ids_hsa.txt") %>% filter(Gene.type %in% c("Mt_rRNA", "protein_coding", "rRNA"))
...
sampleTable <- read.table("../data/metadata/sample_metadata_1.csv", 
                          sep = ",", header = TRUE) %>%
...
# Resultds dds4 intercept

resTC <- results(ddsTC)
```

#### Likelihood ratio test to check for interaction between dose and time
...
test <- as.tibble(merge(neg_d2, pos_d2, by=0, all=T, suffix = c("_neg", "_pos"))) 

test %>%
  filter((log2FoldChange_neg > 0 & log2FoldChange_pos < 0), padj_neg < 0.05) %>%
  pull(Row.names)
...
output_folder <- "~/Desktop/orebro_study/data/machine_learning_dataset/protein_coding_degs" 

for (i in seq_along(degs_names)) {
  filename <- paste0(output_folder, degs_names$names, i, ".csv")
  write.csv(degs_names[[i]], file = filename, row.names = FALSE)
}

for (name in names(degs_names)) {
  filename <- file.path(output_folder, paste0(name, ".csv"))
  write.csv(degs_names[[name]], file = filename, row.names = FALSE)
}
...
Using the MgSigDB Reactome modules 

```{r, gsea_heatmap}
my_palette <- colorRampPalette(c("#265891", "white", "#B80F20"))(n = 20)
sel_list_gse_h <- list_gse_reactome[c(
  "Naïve Dose 1 0-24 hours",
  "Naïve Dose 2 0-24 hours",
  "Naïve Dose 3 0-24 hours",
  "Naïve Dose 3 0-48 hours",
...
combined_gsea_h <- data.table::rbindlist(lapply(sel_list_gse_h, as.data.frame), idcol = TRUE) %>%
  group_by(.id) %>%
  arrange(desc(NES)) %>%
  ungroup()

to_heatmap <- combined_gsea_h %>%
  select(.id, ID, NES) %>%
  filter(NES > 2 | NES < -2, !grepl("TBA", ID)) %>%
  tidyr::pivot_wider(values_from = NES, names_from = ID) %>%
  mutate(across(where(anyNA), ~ tidyr::replace_na(., 0))) %>%
...
Reactome_heatmap <- matrix(as.numeric(to_heatmap), ncol = ncol(to_heatmap)) %>%
    pheatmap::pheatmap(
      width = 100,
      height = 400,
      cellwidth = 20,
...
# save BTM heatmap
   ggsave(
      filename = paste0("../", figures.dir,"Reactome_heatmap.pdf"),
      height = 8, width = 10,
      plot = Reactome_heatmap)
```
...
gene_names_filt <- gene_names[c("Naïve Dose 1 0-24 hours", "Naïve Dose 2 0-24 hours", 
...
g1 <- filtered_gene_doses %>% 
  ...
  theme(axis.ticks = element_line(size = .5), legend.position = "top",
        aspect.ratio = 1)

g1

ggsave(g1,filename = paste0("../", result.dir,"linear_relationship_fold_change_main.pdf"),
      height = 7, width = 7)
```
...
ggsave(g2,filename = paste0("../", result.dir,"linear_relationship_fold_change_supp.pdf"),
      height = 7, width = 7)
```

```{r}
degs <- lapply(gene_names_filt, function(x) x %>% filter(padj < 0.05, log2FoldChange > 1) %>% pull(gene_symbol))
...
row_col_time <- c("Dose 1 0-24 hours" = "#5e4fa2", 
...
heatmap1 <- Heatmap(filtered_degs_matrix_ordered,  border = F, 
                    rect_gp = gpar(type = "none"), column_dend_side = "bottom",
                    show_row_names = FALSE, show_column_names = FALSE,
                    name = "Jaccard similarity index",
                    right_annotation = row_anno, col = my_palette,
...
```{r}
ls_gene_file <- list.files("../data/gene_sets", pattern = "231102", full.names = TRUE)
ls_gene_sets <- lapply(ls_gene_file, read.csv, header = TRUE)
...
cytokine_heatmap <- gene_names_sign_df %>%
  ...
  mutate(group = ifelse(group == "Na", "Naïve", group),
         group_timepoint = paste(timepoint, group)) %>%
  group_by(gene_symbol, timepoint) %>%
  mutate(FoldChange_mean = case_when(degs_presence == "Both significant" ~ mean(FoldChange),
...
g2 <- cytokine_heatmap %>%
  ...
  filter(!all(degs_presence == "non_significant"),
...
g2
ggsave(g2,filename = paste0("../", result.dir,"cytokine_degs.pdf"),
      height = 7, width = 7)
```
...
ggsave(filename = paste0("../", result.dir,"cxcl10_cytokine_degs.pdf"),
      height = 5, width = 5)

plot_counts(gene_interest = "IL16")
ggsave(filename = paste0("../", result.dir,"IL16_degs.pdf"),
      height = 5, width = 5)

plot_counts(gene_interest = "CD40LG")
ggsave(filename = paste0("../", result.dir,"CD40LG_degs.pdf"),
      height = 5, width = 5)

plot_counts(gene_interest = "CCL5")
ggsave(filename = paste0("../", result.dir,"CCL5_degs.pdf"),
      height = 5, width = 5)

plot_counts(gene_interest = "TNF")
ggsave(filename = paste0("../", result.dir,"TNF_degs.pdf"),
      height = 5, width = 5)

plot_counts(gene_interest = "TNFSF13B")
ggsave(filename = paste0("../", result.dir,"TNFSF13B_degs.pdf"),
      height = 5, width = 5)

plot_counts(gene_interest = "IL1B")
ggsave(filename = paste0("../", result.dir,"IL1B_degs.pdf"),
      height = 5, width = 5)

plot_counts(gene_interest = "IL15")
ggsave(filename = paste0("../", result.dir,"IL15_degs.pdf"),
      height = 5, width = 5)
```
...
## map names with TFs
ora_tf <- lapply(degs, function(x) enricher(x, pvalueCutoff = 1,
           pAdjustMethod = "fdr", 
           qvalueCutoff = .2, 
...
g3 <- ora_tf_df_bind %>%  
  ...
  theme(axis.ticks = element_line(size = .5), legend.position = "right",
        ...
        legend.title = element_text())
g3
ggsave(g3, filename = paste0("../", result.dir,"transcription_factors_ORA.pdf"),
      height = 7, width = 7)
```
...
ora_tf_compare <- compareCluster(degs, fun = "enrichPathway")
...
p4 + scale_fill_npg() 
cnetplot(xx, )
ggsave(filename = paste0("../", result.dir,"example_tf_degs.pdf"),
      height = 10, width = 10)
```
...
```{r}
tpm_gene <- data.table::fread("../data/processed_data_nfcore/salmon/salmon.merged.gene_tpm.tsv",  header = TRUE, sep = "\t") %>% 
  as.data.frame()
