...
results_list <- list(res_neg_dose1, res_pos_dose1, res_neg_dose2, res_pos_dose2, res_pos, res_neg, res_dose1, res_dose2)

names(results_list) <- c("neg_dose1", "pos_dose1", "neg_dose2", "pos_dose2","pos","neg", "dose1", "dose2") 
...
.volcano_plot <-  function(x){
  ...
  geom_label_repel(data = subset(x %>% tibble::rownames_to_column("gene_symbol"), abs(log2FoldChange) > 10|padj < 0.05), aes(log2FoldChange,-log10(padj),label= gene_symbol), max.overlaps = 10) +
  xlim(-20,20) +
  ylim(0,10) +
  cowplot::theme_cowplot() +
...
volcano_plots <- lapply(results_list_plot, .volcano_plot)
lapply(names(volcano_plots), function(x) ggsave(filename = paste0("../", figures.dir, x,"_volcano_plot.pdf"), 
                                         plot = volcano_plots[[x]]))
lapply(results_list_plot, .volcano_plot)
...
enrichment_list_down <- lapply(results_list_plot, function(x) {
  ...
  databases = c("GO_Biological_Process_2021", "KEGG_2021_Human","GO_Molecular_Function_2021"))})
#Testing Blood Transcriptome modules
...
gmt_explosion <- enricher(rownames(res_pos[res_pos$padj < 0.05, ]), TERM2GENE = gmt)
...
enrichment_list <- enrichment_list[-c(1)]
...
original_gene_list <- res_neg_dose2$log2FoldChange
# name the vector
names(original_gene_list) <- rownames(res_neg_dose2)
# omit any NA values 
...
Helpful to interpret up/down-regulated pathways.
```{r fig.width=18, fig.height=12}
gseKEGG(gmt_e) + labs(x = "enrichment distribution")
```
...
dme <- pathview(gene.data=kegg_gene_list, pathway.id="hsa03015", species = kegg_organism)
# Produce a different plot (PDF) (not displayed here)
dme <- pathview(gene.data=kegg_gene_list, pathway.id="hsa03015", species = kegg_organism, kegg.native = F)
```
