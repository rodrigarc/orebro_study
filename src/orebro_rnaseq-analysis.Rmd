...
names(results_list) <- c("Baseline Dose 1", "Baseline Dose 2", "Baseline Conv", "Baseline Naïve", "Naïve Dose 1", "Conv Dose 1", "Naïve Dose 2", "Conv Dose 2", "Conv","Naïve", "Dose 1", "Dose 2") 
  ...
  x <- x %>% mutate(test_padj = case_when(padj < 0.05 & log2FoldChange >= 1 ~ "Up regulated",
                    ...
                    test_pvalue = case_when(padj < 0.05 & log2FoldChange >= 1 ~ "Up regulated",
                                     padj < 0.05 & log2FoldChange <= -1 ~ "Down regulated",
                                     TRUE ~ "Not significant"),
                    )
  })
...
## Function for plotting over representation plots

plots_op <- function(list){
  plots_enrich <- list()
for(pathway in 1:length(list[[1]])){
  
  rbinded_enrich_list <- lapply(list, function(x)x[[pathway]])
  print(names(list[[1]])[[pathway]])
...
plots_op(enrichment_list_up)
plots_op(enrichment_list_down)
...
enrichment_list_up[c("Baseline Dose 1", "Baseline Dose 2", "Conv", "Naïve", "Dose 1", "Dose 2")] <- NULL
...
up_plots <- lapply(plots_enrich, function(x){
  ...
  ggplot(aes(x = Term, y = -log10(Adjusted.P.value))) +
    geom_bar(stat = "identity", fill = "dark red", width = .5) +
    theme_minimal() +
    theme(text = element_text(size = 10),
          plot.title = element_text(hjust = (5 / nchar(results_list)) * 2), 
          plot.margin = margin(t = 5, r = 50, b = 5, unit = "pt"), 
...
go_plot <- (GO_bio_up / GO_bio_down) | (GO_mol_up / GO_mol_down)
go_plot <- go_plot + plot_annotation(tag_levels = "a")

go_plot

ggsave(filename = paste0("../", figures.dir, "go_plot.png"), plot = go_plot)
```
