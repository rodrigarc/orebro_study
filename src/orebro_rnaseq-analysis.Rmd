...
```{r}
#reading data
countTable <- data.table::fread("../data/proces-data_core-facility/subreadCounts_hg38ens_minus_frag.txt", header = TRUE, sep = "\t") %>%  as.data.frame()
...
countTable <- countTable %>%
  ...
  dplyr::mutate(Geneid = plyr::mapvalues(x = countTable$Geneid, from = edb_genes$gene_id, to = edb_genes$symbol, warn_missing = FALSE))

## aggregate and sum up genes with same gene symbol, which were basically non-coding RNAs
countTable <- aggregate(countTable[,-1], list(Geneid=countTable[,1]), FUN = sum)
rownames(countTable) <- countTable$Geneid
countTable <- dplyr::select(countTable, -c(Geneid))
...
countTable <- countTable[, pmatch(sampleTable$sample_ID, colnames(countTable))]
#feel free to change smaple name accoding to what you think is best
colnames(countTable) <- sampleTable$sample_ID

#Visualize low counts
