...
## renaming genes based on emsembl
...
#  dplyr::mutate(Geneid = plyr::mapvalues(x = countTable$Geneid, from = G_list$ensembl_gene_id, to = G_list$hgnc_symbol,warn_missing = FALSE))

rownames(countTable) <- countTable$Geneid
countTable <- countTable %>% dplyr::select(-c(Geneid,Chr, Start, End, Strand, Length))

sampleTable <- read.table(("..data/proces-data_core-facility/sample_metadata.csv", as.is = TRUE, header = FALSE, sep = "\t") 
