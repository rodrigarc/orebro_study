...
```{r}
# count data
count_dir <- file.path(data_dir, "processed_data_nfcore/batch_3_selfprocessed")
count_file <- file.path(count_dir, "salmon.merged.gene_counts.tsv")

# metadata
sample_metadata_file <- file.path(sample_metadata_dir, "sample_metadata_3.csv")
# NGI metadata
sample_metadata_NGI <- file.path(sample_metadata_dir, "BEA23P140_translation.xlsx")
```
...
# Filter for 3rd batch 10 error samples
```{r, results='hide'}
# Load NGI sample metadata
sample_metadata_NGI <- readxl::read_excel(sample_metadata_NGI) 

# change first row to be column names
colnames(sample_metadata_NGI) <- sample_metadata_NGI[1, ]  

# remove first row
sample_metadata_NGI <- sample_metadata_NGI[-1, ] %>% 
  # change name of columns
  rename(
    sample_ID = "NGI Sample ID", # matches with counts_raw colnames
    sample = 'sample_id', # matches with sampleTable sample column
  )

# filter sampleTable to only include NGI samples that match with sample_metadata_NGI sample column
sampleTable <- sampleTable %>% 
  filter(sample %in% sample_metadata_NGI$sample)
```

# remove control samples
```{r}
# remove subject_ID 222-19 and 222-8
sampleTable <- sampleTable %>% 
  filter(subject_ID != "222-19" & subject_ID != "222-8")
```


```{r}
# Adjust the column names by removing the last underscore and everything after it
colnames(counts_raw) <- sub("_S[0-9]+$", "", colnames(counts_raw))
```
...
#   countData = round(counts_filtered),
#   colData = as.data.frame(sampleTable),
#   design = ~ age + batch + time
# )
# 
# dds3 <- DESeqDataSetFromMatrix(
#   countData = round(counts_filtered),
#   colData = as.data.frame(sampleTable),
#   design = ~ age + batch + dose
# )
# 
# dds4 <- DESeqDataSetFromMatrix(
#   countData = round(counts_filtered),
#   colData = as.data.frame(sampleTable),
#   design = ~ age + batch + dose + group + dose:group
)
```
...
```{r, results='hide', fig.width=18, fig.height=12, fig.keep=10:17}
results_list <- list(
  pos_d2
)
...
Database used for enrichment analysis, "Reactome". Also testing the Blood Transcriptome modules presented in [Li et al. 2014](10.1038/ni.2789).
...
Gene set enrichment analysis based on fold change ranking using the blood transcriptome modules presented in [Li et al. 2014](10.1038/ni.2789). Cut-off; FDR-adjusted p-value \< 0.05. N = 15
...
```{r, include=FALSE}
# save supplementary fold change comparison plot
ggsave(g2,
...
```{r, include=FALSE}
# save transcription factor ORA plot
ggsave(g3,
